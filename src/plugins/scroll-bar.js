...
(function($){
    $.fn.extend({
        "scrollBar":function(options){
            if(typeof options === "string" && $(this).data('scrolled')){ // opreations
                switch (options) {
                    case 'getScrollBarStatus':
                        var $scrollBarx = $(this).find('.zl-scrollBar.zl-horizontalBar')
                        var $scrollBary = $(this).find('.zl-scrollBar.zl-verticalBar')
                        return{
                            x:!!$scrollBarx.length,
                            y:!!$scrollBary.length
                        }
                ...
                switch (settings.position){
                    ...
                    case "x,y":
                        if(scrollH>innerH && scrollW>innerW){
                            ...
                            $(obj).find(".zl-verticalBar").height(barHeight).parent().height(outerH+paddingR);
                            ...
                            //paddingR + settings.barWidth
                            $(".zl-horizontalBar-after",$(obj)).remove()
                            $(obj).find(".zl-horizontalBar").width(barWidth).parent().width(outerW + paddingR )
                                .css({
                                    "padding-right":settings.barWidth+"px",
                                    // "box-sizing":"border-box" // fixed
                                })
                                .after(
                                    '<div class="zl-horizontalBar-after" style="position:absolute;right:0px;bottom:0;width:'+(settings.barWidth+1)+'px;height:'+(settings.barWidth+1)+'px;background-color:#DCDCDC"></div>'
                                );
                ...
                switch (direction){
                    ...
                    case "0":
                        ...
                        document.onmousemove = function(ev){
                            ...
                            var contentDivTop = -(topCount*(scrollH - innerH)/height)
                            contentDiv.style.top =  contentDivTop + 'px';
                            options.onSrollY && options.onSrollY(contentDiv.children[0],contentDivTop)
                    ...
                    case "1":
                        ...
                        document.onmousemove = function(ev){
                            ...
                            var contentDivLeft = -(leftCount*(scrollW - innerW)/width)
                            contentDiv.style.left = contentDivLeft + 'px';
                            options.onSrollX && options.onSrollX(contentDiv.children[0],contentDivLeft)
                        };
            ...
            function fn(ev,a){
                if(a.find(".zl-verticalBar").length){
                    ...
                    var contentDivTop = -(topDis*(scrollH - innerH)/height)
                    a.find(".zl-scrollBar").get(0).style.top = topDis  + "px";
                    a.find(".zl-scrollContentDiv").get(0).style.top = contentDivTop + 'px';
                    options.onSrollY&&options.onSrollY(contentDiv,contentDivTop)
                }
            };
