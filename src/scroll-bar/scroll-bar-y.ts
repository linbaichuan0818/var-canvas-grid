...
import {  moveScrollBarYCallBack, scrollBarYOnClickCallBack  } from "../event/index";
...
export class ScrollBarY extends BaseBar{
    ...
    private paintTopbtn(){
        ...
        const originX = w / 4;
        const originY = h / 4;
        this.drawTriangle(x + 2*originX, y + originY, 
                          x + originX, y + h - originY, 
                          x + w - originX, 3*originY, 
                          "#A8A8A8", "fill" );
    }
    ...
    private paintBottomtbtn(){
        ...
        this.drawTriangle(x + originX, y + originY, 
                          x + 2*originX, y + h - originY, 
                          x + w - originX, y + (ch-y)/2 - 3*originY, 
                          "#A8A8A8", "fill" );
    }
